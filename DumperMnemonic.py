...
def process_address(queue, words, eth, doge, bnb, util):
    while True:
        mnemonic = queue.get()
        if mnemonic is None:
            break
        ...
        bnb_addr = bnb.hex_addr(convert_hex)

        eth_bal = CheckBalanceEthereum(eth_addr)
        doge_bal = CheckBalanceDogecoin(doge_addr)
        bnb_bal = CheckBalanceBNB(bnb_addr)

        if eth_bal == "error" or doge_bal == "error" or bnb_bal == "error":
        ...
        if bnb_balance > 0:
            ...
            titler(f"Gen: {z} / Con: {ff} / USD: {usd} $")
            with open("found.txt", "a") as dr:
                dr.write(f"BNB: {bnb_addr} | Balance: {bnb_balance}\n"
                         f"Mnemonic: {mnemonic}\n"
                         f"Private Key: {convert_hex}\n")
    else:
        ...
        print(f"[{z} | Found:{ff}]  BNB: {green}{bnb_addr}{reset}{spc_bnb}[Balance: {green}{bnb_balance}{reset}]")
...
if __name__ == "__main__":
    ...
    # Generate mnemonic_list
    mnemonic_list = [" ".join(random.choice(words) for _ in range(random.choice([12, 24]))) for _ in range(100)]
    ...
    for _ in range(num_processes):
        p = Process(target=process_address,
